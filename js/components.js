...
function createAdwDialog(options = {}) {
  ...
  // --- Show/Hide Logic ---
   function openAdwDialog() {
      document.body.appendChild(backdrop); // Append to body *here*
      backdrop.appendChild(dialog);
      setTimeout(() => { // Allow for transitions
          backdrop.classList.add('open');
          dialog.classList.add('open');
      }, 10); // Use a small timeout for transition
  }
  function closeAdwDialog() {
    ...
    setTimeout(() => {
      backdrop.remove(); // Remove the backdrop *and* dialog
        if (options.onClose) {
          options.onClose();
        }
  ...
  return {
      dialog: dialog,  // The dialog element itself (might not be needed directly)
      open: openAdwDialog,  // Method to open the dialog
      close: closeAdwDialog, // Method to close the dialog
  };
}
