...
from app_demo.models import User, Post, Notification, Activity
...
def test_list_notifications_authenticated_with_unread_and_read(client, logged_in_client, db, create_test_user, create_test_post):
    ...
    current_user = User.query.filter_by(username="loginuser").first() # from logged_in_client
    actor = create_test_user(username="actor_user_notif", email="actor_n@example.com")
    ...
    other_user = create_test_user(username="other_user_notif", email="other_n@example.com")
...
def test_list_notifications_pagination(client, logged_in_client, app, db, create_test_user, create_test_post):
    ...
    current_user = User.query.filter_by(username="loginuser").first()
    actor = create_test_user(username="actor_notif_pager", email="anp@example.com")
...
def test_mark_notification_as_read_successful(client, app, logged_in_client, db, create_test_user):
    ...
    current_user = User.query.filter_by(username="loginuser").first()
    actor = create_test_user(username="actor_mark_read", email="amr@example.com")
    unread_notif = create_test_notification(db, current_user, actor, type="to_be_read", is_read=False)

    assert not unread_notif.is_read

    with app.app_context(): # For CSRF
        from flask_wtf import FlaskForm; class D(FlaskForm):pass; token = D().csrf_token.current_token

    response = logged_in_client.post(url_for('notification.mark_as_read', notification_id=unread_notif.id), data={'csrf_token': token}, follow_redirects=True)
    assert response.status_code == 200
...
def test_mark_notification_as_read_unauthenticated(client, db, create_test_user):
    ...
    user = create_test_user(username="user_for_notif_unauth", email="ufnu@example.com")
    actor = create_test_user(username="actor_for_notif_unauth", email="afnu@example.com")
    notif = create_test_notification(db, user, actor, is_read=False)

    response = client.post(url_for('notification.mark_as_read', notification_id=notif.id), data={'csrf_token':'dummy'}, follow_redirects=True)
    assert response.status_code == 200
    assert b"Please log in to access this page." in response.data
...
def test_mark_notification_as_read_not_own_notification(client, logged_in_client, app, db, create_test_user):
    ...
    other_user = create_test_user(username="other_user_for_notif", email="oufn@example.com")
    actor = User.query.filter_by(username="loginuser").first() # Current user is actor
    ...
    response = logged_in_client.post(url_for('notification.mark_as_read', notification_id=other_users_notif.id), data={'csrf_token':token})
...
def test_mark_notification_as_read_already_read(client, app, logged_in_client, db, create_test_user):
    ...
    current_user = User.query.filter_by(username="loginuser").first()
    actor = create_test_user(username="actor_already_read", email="aar@example.com")
    read_notif = create_test_notification(db, current_user, actor, is_read=True)

    with app.app_context(): from flask_wtf import FlaskForm; class D(FlaskForm):pass; token = D().csrf_token.current_token
    response = logged_in_client.post(url_for('notification.mark_as_read', notification_id=read_notif.id), data={'csrf_token':token}, follow_redirects=True)
    assert response.status_code == 200
...
def test_mark_notification_as_read_csrf_missing(client, logged_in_client, db, create_test_user):
    ...
    current_user = User.query.filter_by(username="loginuser").first()
    actor = create_test_user(username="actor_csrf_read", email="acsr@example.com")
    unread_notif = create_test_notification(db, current_user, actor, is_read=False)

    response = logged_in_client.post(url_for('notification.mark_as_read', notification_id=unread_notif.id), data={})
...
def test_mark_all_notifications_as_read_successful(client, app, logged_in_client, db, create_test_user):
    ...
    current_user = User.query.filter_by(username="loginuser").first()
    actor = create_test_user(username="actor_mark_all", email="ama@example.com")
    ...
    with app.app_context(): from flask_wtf import FlaskForm; class D(FlaskForm):pass; token = D().csrf_token.current_token
    response = logged_in_client.post(url_for('notification.mark_all_as_read'), data={'csrf_token':token}, follow_redirects=True)
    assert response.status_code == 200
...
def test_mark_all_notifications_as_read_no_unread(client, app, logged_in_client, db, create_test_user):
    ...
    current_user = User.query.filter_by(username="loginuser").first()
    actor = create_test_user(username="actor_no_unread", email="anu@example.com")
...
def test_mark_all_notifications_as_read_csrf_missing(client, logged_in_client):
    ...
    response = logged_in_client.post(url_for('notification.mark_all_as_read'), data={})
